...
Include `source_row_id` when retrieving data: this ingestion locator prevents the connector from grouping identical records and summing their numeric values. It is not a business deduplication key. Join by `order_id` and confirm `journey_id`. Deduplicate orders by order ID and touches by touch ID, ignoring the differing ingestion locators. Retain completed orders in the campaign window. Eligible paid clicks must occur no later than conversion and within the preceding seven days, inclusive. Direct-only orders remain unattributed to paid media. Empty order IDs identify non-converting journeys.
