...
for(const [name,rows]of Object.entries({'channel-daily':spend,orders,touchpoints:touches})){rows.forEach((r,i)=>r.source_row_id=name+'-'+String(i+1).padStart(4,'0'));fs.writeFileSync(path.join(root,'data',name+'.csv'),csv(rows));fs.writeFileSync(path.join(root,'data',name+'.json'),JSON.stringify(rows,null,2)+'\n');}
